...
{
	const int FFT_FRAME_SIZE = 512;			
	const int HALF_FFT_FRAME_SIZE = 256;
	const int NFORMANTS = 4;				
	FLOAT *FFT_result;					
	...
	{
		pos_temp = (UINT32) roundInt(( ((FLOAT) final_pitch[pos_pitch]) / HALF_FFT_FRAME_SIZE) );
		...
		spec_unit = (INT32) roundInt((FFT_FRAME_SIZE / (FLOAT) pitch_length));
		half_spec_unit = roundInt( (FLOAT) spec_unit /2);
		quarter_spec_unit = roundInt( (FLOAT) spec_unit /4);
