...
INT32 i;
	...
	if(ErrorCode==LPCBUFFER_OK)
	{
		...
		if(PHand->pdLPCTimeBuffer==NULL)ErrorCode=(LPCBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;

		PHand->pdLPCTimeWindow=NULL;
		PHand->pdLPCTimeWindow=calloc(lLPCWindowSize,sizeof(FLOAT));
		if(PHand->pdLPCTimeWindow==NULL)ErrorCode=(LPCBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;

		PHand->pdLPCWindTimeSignal=NULL;
		PHand->pdLPCWindTimeSignal=calloc(lLPCWindowSize,sizeof(FLOAT));
		if(PHand->pdLPCWindTimeSignal==NULL)ErrorCode=(LPCBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;
