...
FILTERBUFFER_HANDLE PHand=*phBuff;
	...
	if(ErrorCode==FILTERBUFFER_OK)
	{
		...
		if(PHand->pdTimeBuffer==NULL)ErrorCode=(FILTERBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;
		PHand->lTimeBufferSize=lSignalSize;


		PHand->pFrameData=calloc(PHand->lMaxNrFrames,sizeof(FRAME_DATA));
		if(PHand->pFrameData==NULL)ErrorCode=(FILTERBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;

		PHand->pdResiduumBuffer=calloc(lSignalSize,sizeof(FLOAT));
		if(PHand->pdResiduumBuffer==NULL)ErrorCode=(FILTERBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;
		PHand->lResiduumBufferSize=lSignalSize;

		PHand->pdSynthBuffer=calloc(lSignalSize,sizeof(FLOAT));
		if(PHand->pdSynthBuffer==NULL)ErrorCode=(FILTERBUFFER_ERRORCODE)LPCANALYSIS_MEMORY_ALLOCATION_FAILED;
